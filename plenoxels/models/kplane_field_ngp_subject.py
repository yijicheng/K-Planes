...
class GridsGenerator(torch.nn.Module):
    def __init__(self, config, multiscale_res_multipliers, concat_features):
        super().__init__()
        self.config = config
    ...
    def forward(self, z):
        assert self.config["resolution"][0] == self.config["resolution"][2]
        assert self.config["resolution"][1] == self.config["resolution"][2]
        coo_combs = list(itertools.combinations(range(self.config["input_coordinate_dim"]), self.config["grid_dimensions"]))
        planes = self.backbone(z)
        planes = planes.view(
            len(coo_combs),
            self.config["output_coordinate_dim"],
        ...
        for res in self.multiscale_res_multipliers:
            ...
            self.config["resolution"] = [
                r * res for r in self.config["resolution"][:3]
            ] + self.config["resolution"][3:]
            gp = load_grid_param(
                latent_chunk=torch.chunk(planes, chunks=3, dim=0),
                grid_nd=self.config["grid_dimensions"],
                in_dim=self.config["input_coordinate_dim"],
                out_dim=self.config["output_coordinate_dim"],
                reso=self.config["resolution"],
            )
...
class KPlaneField(nn.Module):
    def __init__(
        ...
        self.feature_dim = config["output_coordinate_dim"]
        self.grids_generator = GridsGenerator(config, self.multiscale_res_multipliers, self.concat_features)
    ...
    def get_grids(self, latent):
        self.grids, _ = self.grids_generator(latent)

    def density_fn(self, positions: torch.Tensor, timestamps: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        """
        if timestamps is not None and (len(positions.shape) == 3 and len(timestamps.shape) == 2):
            # position is [ray, sample, 3]; times is [ray, 1]
            timestamps = timestamps[:, None]  # RaySamples can handle the shape # type: ignore
        ...
        density, _ = self.get_density(ray_samples.get_positions(), timestamps)
        return density
    ...
    def get_density(self, pts: torch.Tensor, timestamps: Optional[torch.Tensor] = None):
        ...
        ray_samples_shape = pts.shape[:-1]
        if timestamps is not None:
            raise ValueError("Dynamics version has not been implemented for NGPModel.")
            n_rays, n_samples = ray_samples_shape
            timestamps = timestamps[:, None].expand(-1, n_samples)[..., None]  # [n_rays, n_samples, 1]
            pts = torch.cat((pts, timestamps), dim=-1)  # [n_rays, n_samples, 4]

        pts_flatten = pts.reshape(-1, pts.shape[-1])
        features = interpolate_ms_features(
            pts_flatten, ms_grids=self.grids,  # noqa
    ...
    def forward(self,
        ...
        if self.use_appearance_embedding:
            raise ValueError("Apperance version has not been implemented for NGPModel.")
        ...
        if self.linear_decoder:
            ...
            rgb = torch.sigmoid(rgb).view(*ray_samples_shape, 3)
        else:
            rgb = self.color_net(color_features).to(directions).view(*ray_samples_shape, 3)
